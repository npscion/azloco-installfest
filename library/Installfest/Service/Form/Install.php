...
class Installfest_Service_Form_Install implements Installfest_Interface_FormServiceInterface
{
    ...
    public function processForm($form)
    {
        try {
            ...
            $total = count($form_values);
            for($i = 0; $i < $total; $i++)
            {
                if(empty($form_values[key($form_values)])) {

                    $form_values[key($form_values)] = "N/A";
                }
                next($form_values);
            }
            ...
            $sql = "INSERT INTO install_catalog (submit_date, name, helper_name, email, computer_make, computer_model_number, memory_size, disk_size, disk_free_space, backup, old_os, requested_os, dual_boot, disk_cleanup, disk_defrag, cpu_model_number, gpu_model_number, wifi_model_number, ethernet_model_number, installed_os, installed_os_version, notes, lightweight_linux, updated, user_confirmation, resource_list)
            VALUES ('$date', '$name', '$helper_name', '$email', '$computer_make', '$computer_model_number', '$memory_size', '$disk_size', '$disk_free_space', '$backup', '$old_os', '$requested_os', '$dual_boot', '$disk_cleanup', '$disk_defrag', '$cpu_model_number', '$gpu_model_number', '$wifi_model_number', '$ethernet_model_number', '$installed_os', '$installed_os_version', '$notes', '$lightweight_linux', '$updated', '$user_confirmation', '$resource_list')";

            if ($connect->query($sql) == TRUE) {
                echo "Submission was successful<br></br>";
            } else {
                echo "Error ";
                Zend_Debug::dump(mysqli_errno($connect) . ": " . mysqli_error($connect));die;
            }
